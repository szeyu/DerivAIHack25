...
# os.system("pip install chromadb unstructured pdfminer.six sentence-transformers nltk requests langchain python-dotenv")
...
class DataExtractor:
    """
    Extracts and preprocesses dispute-related data from various sources using Langchain components.
    """
    ...
    def pdf_to_semantic_chunks(self, pdf_path: str) -> List[str]:
        """
        Converts unstructured PDF data into semantic chunks using Langchain's PyPDFLoader and RecursiveCharacterTextSplitter.
        """
        try:
            loader = PyPDFLoader(pdf_path)
            documents = loader.load()
            chunks = self.text_splitter.split_documents(documents)
            return [chunk.page_content for chunk in chunks]  # Return list of strings
...
class KnowledgeRetriever:
    """
    Retrieves relevant knowledge from the vector database using Langchain components.
    """
    ...
    def __init__(self, collection_name: str = "dispute_resolution_kb", embedding_model_name: str = 'all-mpnet-base-v2'):
        ...
        self.embedding_model = HuggingFaceEmbeddings(model_name=embedding_model_name)  # Langchain embeddings
        self.persist_directory = "./chroma_db" #Local persist directory

        # Initialize ChromaDB
        self.vectordb = Chroma(persist_directory=self.persist_directory, embedding_function=self.embedding_model, collection_name=self.collection_name)

        # Check if the collection exists
        if not os.path.exists(os.path.join(self.persist_directory, "index")):
    ...
    def hybrid_search(self, query: str, k: int = 5) -> List[str]:
        """
        Performs hybrid search using Langchain's VectorStoreRetriever.
        This simplified version focuses on vector search.  Keyword search can be added but is not implemented here for brevity.
        """
    ...
    def retrieve_knowledge(self, query: str) -> List[str]:
        """
        Main function to retrieve knowledge, using the hybrid search.  Adaptive retrieval and early stopping are possible to implement with more complex logic.
        """
    ...
    def add_data_to_db(self, documents: List[str], metadatas: List[Dict[str, Any]] = None, ids: List[str] = None):
        ...
        from langchain.document_loaders import TextLoader #Import here to avoid circular dependency
        langchain_documents = []
        for i, doc_content in enumerate(documents):
            # Create a dummy file for each document
            temp_file_path = f"temp_doc_{i}.txt"
            with open(temp_file_path, "w") as temp_file:
                temp_file.write(doc_content)
            loader = TextLoader(temp_file_path)
            document = loader.load()[0]  # Load the document
            os.remove(temp_file_path) #Clean up
            langchain_documents.append(document)

        self.vectordb.add_documents(langchain_documents)
        self.vectordb.persist() #Persist to disk
    ...
    def get_qa_chain(self, llm):
        ...
        qa_chain = RetrievalQA.from_chain_type(
            ...
            retriever=retriever,
            return_source_documents=False  # Set to True to return source documents
        )
        return qa_chain
...
class DisputeResolver:
    """
    Analyzes the dispute context and retrieved knowledge to propose resolutions using Langchain.
    """

    def __init__(self, knowledge_retriever: KnowledgeRetriever, llm): #LLM passed in
        self.knowledge_retriever = knowledge_retriever
        self.llm = llm

    def initial_analysis(self, dispute_context: str) -> str:
        """
        Identifies core issues and conflict points in the dispute context using an LLM.
        """
        prompt = f"""
        ...
        """
        return self.llm(prompt)  # Use Langchain LLM directly

    def solution_generation(self, dispute_context: str, retrieved_knowledge: List[str]) -> str:
        """
        Proposes potential resolutions based on retrieved knowledge using an LLM.
        """
    ...
    def self_verification(self, initial_solution: str, dispute_context: str, retrieved_knowledge: List[str]) -> str:
        """
        Checks for contradictions or missing knowledge in the initial solution using an LLM.
        """
        knowledge_string = "\n".join(retrieved_knowledge)
        prompt = f"""
...
class JudgeAgent:
    """
    Evaluates the coherence, fairness, and completeness of the proposed resolution using Langchain.
    """

    def __init__(self, knowledge_retriever: KnowledgeRetriever, llm):
        self.knowledge_retriever = knowledge_retriever
        self.llm = llm

    def evaluate_solution(self, dispute_context: str, proposed_solution: str) -> Dict[str, Any]:
        """
        Evaluates the coherence, fairness, and completeness of the proposed solution using an LLM.
        """
        prompt = f"""
        ...
        """
        response = self.llm(prompt) #Call to LLM

        #Simple parsing assuming the LLM will return something like:  {"confidence": 0.8, "justification": "..."}
        try:
...
class LLMProvider:
    def __init__(self, model_name: str):
        ...
        #     self.llm = SomeDeepSeekWrapper()  # Placeholder for actual DeepSeek API wrapper
...
def main():
    ...
    """
    # 0. Initialize LLM (Crucial: Replace with your desired LLM and API key)
    ...
    knowledge_retriever.add_data_to_db(documents=initial_knowledge) # Add all documents

    # 3. Primary Reasoning Stage
    dispute_resolver = DisputeResolver(knowledge_retriever, llm)
    initial_solution = dispute_resolver.resolve_dispute(user_data["dispute_context"])

    # 4. Adjudication & Quality Control
    judge_agent = JudgeAgent(knowledge_retriever, llm)
